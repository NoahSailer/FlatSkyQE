...
#!!! manuwarning
      ...
      # TT
      if fg and not atm:
         self.ftotalTT = lambda l: self.flensedTT(l) + self.fkSZ(l) + self.fCIB(l) + self.ftSZ(l) + self.ftSZ_CIB(l) + self.fradioPoisson(l) + self.fgalacticDust(l) + self.fdetectorNoise(l)
      elif fg and atm:
         self.ftotalTT = lambda l: self.flensedTT(l) + self.fkSZ(l) + self.fCIB(l) + self.ftSZ(l) + self.ftSZ_CIB(l) + self.fradioPoisson(l) + self.fgalacticDust(l) + self.fdetectorNoise(l) + self.fatmosphericNoiseTT(l)
      ...
      self.fkSZ = lambda l: a_kSZ * fkSZ_template(l) * self.kszFreqDpdceT(self.nu1) * self.kszFreqDpdceT(self.nu2) / self.kszFreqDpdceT(self.nu0)**2
   ...
   def fatmosphericNoiseTT(self, l):
      ...
      # When nu1 != nu2, the detector noise is 0, when in reality
      # the correlation coefficient for the atmospheric noise is
      # is somewhere around 90%
      def red_noise(freq):
         f_sky = 0.4
         A_SR = 4. * np.pi * f_sky
         survey_time = 5. #years
         t = survey_time * 365.25 * 24. * 3600.
         if freq == 27e9: return 200.*A_SR/t/(2*1.) /0.17
         if freq == 39e9: return 77.*A_SR/t/(2*1.) /0.17
         if freq == 93e9: return 1800.*A_SR/t/(2*4.) /0.17
         if freq == 145e9: return 12000.*A_SR/t/(2*4.) /0.17
         if freq == 225e9: return 68000.*A_SR/t/(2*2.) /0.17
         if freq == 280e9: return 124000.*A_SR/t/(2*2.) /0.17
         return 0.
        
      cc = 0
      con1 = self.nu1 == 27e9  and self.nu2 == 39e9
      con2 = self.nu2 == 27e9  and self.nu1 == 39e9
      con3 = self.nu1 == 93e9  and self.nu2 == 145e9
      con4 = self.nu2 == 93e9  and self.nu1 == 145e9
      con5 = self.nu1 == 225e9 and self.nu2 == 280e9
      con6 = self.nu2 == 225e9 and self.nu1 == 280e9
      con = con1 or con2 or con3 or con4 or con5 or con6
      if con: cc = 0.9  
        
      if self.nu1 == self.nu2: result *= red_noise(self.nu1)
      else: result *= cc * np.sqrt(red_noise(self.nu1)*red_noise(self.nu2)) 
      return result
   ...
   def tszFreqDpdceInt(self, nu):
      ...
      result *= self.dBdT(nu, self.Tcmb) #self.blackbody(nu, self.Tcmb)
      return result
   ...
   def saveFreqDpdce(self):
      ...
      data[:,1] = np.array(self.cmbMonopoleFreqDpdceInt(self.Nu))
      data[:,2] = np.array(self.cmbFluctuationsFreqDpdceInt(self.Nu))
      data[:,3] = np.array(self.kszFreqDpdceInt(self.Nu))
      data[:,4] = np.array(self.tszFreqDpdceInt(self.Nu))
      data[:,5] = np.array(self.cibPoissonFreqDpdceInt(self.Nu))
      data[:,6] = np.array(self.cibClusteredFreqDpdceInt(self.Nu))
      data[:,7] = np.array(self.radioPoissonFreqDpdceInt(self.Nu))
      data[:,8] = np.array(self.galacticDustFreqDpdceInt(self.Nu))

      # Thermo temperature freq dpdces
      f = lambda nu: self.convertIntSITo(nu, kind="tempKcmb")
      intToTemp = np.array(f(self.Nu))
      #
      ...
      intToTempRJ = np.array(f(self.Nu))
      #
   ...
   def genWiggleNoWiggle(self, test=False):
      ...
      ClCmb = np.array(self.funlensedTT(L))
      ...
      ClCmbNoWiggle = np.array(funlensedTTNoWiggle(L))
      ...
      ClCmbWiggleOnly = np.array(funlensedTTWiggleOnly(L))
